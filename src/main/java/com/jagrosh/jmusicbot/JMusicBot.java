...
public class JMusicBot {
  ...
  public static void main(String[] args) {
    ...
    Prompt prompt = new Prompt(
      "JMusicBot",
      "переключаемся в nogui режим. Вы можете сами его включать, сделав -Dnogui=true флаг."
    );
    ...
    if (!System.getProperty("java.vm.name").contains("64") || !(v_num >= 8 && v_num <= 16)) {
      prompt.alert(
        ...
        "Java Version",
        "Вы используете неподдерживаемую версию JAVA. Пожалуйста используйте JAVA 8-16 64-bit."
      );
      System.exit(1);
    }
    // load config
    ...
    AboutCommand aboutCommand = new AboutCommand(
      ...
      new String[] {
        "Воспроизведение данных типов файлов: MP3, MP4, FLAC, WAV, WEBM, OGG, AAC, M3U",
        "Воспроизведение из Youtube, SoundCloud, Vimeo, Twitch",
    ...
    if (config.getStatus() != OnlineStatus.UNKNOWN) cb.setStatus(
      config.getStatus()
    );
    if (config.getGame() == null) cb.useDefaultGame(); else if (
      config.getGame().getName().equalsIgnoreCase("none")
    ) {
      cb.setActivity(null);
      nogame = true;
    ...
    if (!prompt.isNoGUI()) {
      try {
        ...
        log.error(
          "Не удалось запустить GUI. Если вы запускаете " +
    ...
    try {
      ...
      prompt.alert(
        Prompt.Level.ERROR,
        "JMusicBot",
        "Некоторые строчки конфигурационного файла " +
        "не правильные: " +
        ex +
        "\nРасположение config.go: " +
        config.getConfigLocation()
    ...
    log.info(
      "Done (" +
      formatter.format(System.currentTimeMillis() - timer) +
      "s)! For help, type " +
      config.getPrefix() +
      "help in chat"
    );
