...
public class PlayCmd extends MusicCommand {
  ...
  public void doCommand(CommandEvent event) {
    if (
      ...
      if (
        ...
        } else event.replyError("Только DJ могут убирать паузу!");
        return;
      }
      StringBuilder builder = new StringBuilder(
        event.getClient().getWarning() + " Play команды:\n"
      );
      builder
        .append("\n`")
        ...
        .append(name)
        .append(
          " <название пластинки>` - Проигрывает первую пластинку с YouTube"
        );
      builder
        .append("\n`")
        .append(event.getClient().getPrefix())
        .append(name)
        .append(" <URL>` - проигрывает пластинку в данном URL");
      for (Command cmd : children) builder
        .append("\n`")
        .append(event.getClient().getPrefix())
        .append(name)
        .append(" ")
        ...
        if (count == 0) {
          m
            .editMessage(
              FormatUtil.filter(
                event.getClient().getWarning() +
                " Все пластинки в данном плейлисте " +
                (
                  playlist.getName() == null
                    ? ""
                    : "(**" + playlist.getName() + "**) "
                ) +
                "были длиннее чем разрешенный лимит (`" +
            ...
            m
              .editMessage(
                FormatUtil.filter(
                  ...
                  " с `" +
                  playlist.getTracks().size() +
                  "` пластинкой; Он добавлена в очередь!" +
                  (
                    count < playlist.getTracks().size()
                      ? "\n" +
                      event.getClient().getWarning() +
                      "Некоторые пластинки были убраны"
                      : ""
                  )
                )
              )
            ...
            m
              .editMessage(
                FormatUtil.filter(
                  ...
                  " с `" +
                  playlist.getTracks().size() +
                  "` пластинками; Он добавлена в очередь!" +
                  (
                    count < playlist.getTracks().size()
                      ? "\n" +
                      event.getClient().getWarning() +
  ...
  public class PlaylistCmd extends MusicCommand {
    ...
    public void doCommand(CommandEvent event) {
      ...
      if (playlist == null) {
        event.replyError(
          "Плейлист с названием `" + event.getArgs() + ".txt` не существует."
        );
        return;
      }
      event
            ...
            () -> {
              StringBuilder builder = new StringBuilder(
                playlist.getTracks().isEmpty()
                  ? event.getClient().getWarning() +
                  " Пластинки не были загружены!"
                  : event.getClient().getSuccess() + " Пластинки были загружены"
              );
              if (!playlist.getErrors().isEmpty()) builder.append(
