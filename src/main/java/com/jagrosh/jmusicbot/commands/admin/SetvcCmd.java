  ...
  protected void execute(CommandEvent event) {
    ...
    if (event.getArgs().equalsIgnoreCase("none")) {
      ...
      event.reply(
        ...
        " Пластинки теперь могут проигрываться повсюду"
      );
    } else {
      List<VoiceChannel> list = FinderUtil.findVoiceChannels(
        event.getArgs(),
        event.getGuild()
      );
      if (list.isEmpty()) event.reply(
        event.getClient().getWarning() +
        " Нет Войс Каналов с таким названием \"" +
        event.getArgs() +
        "\""
      ); else if (list.size() > 1) event.reply(
        event.getClient().getWarning() +
        FormatUtil.listOfVChannels(list, event.getArgs())
      ); else {
        s.setVoiceChannel(list.get(0));
        event.reply(
          event.getClient().getSuccess() +
          " Пластинки теперь могут проигрываться только в " +
